...
export default function Feedback({ showFeedback }) {
    ...
    const calculateFinalScore = () => {
        ...
        const finalScore = Object.values(scores).reduce((total, score) => total + score, 0) / totalFeedbacks;
                ...
                feedbackData[showFeedback]?.map((label) => (
                    <div key={label} style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                        ...
                        <StarsLine
                            ...
                            isEditable={true}
                            size="lg"
                            onStarClick={(starIndex, newScore) => handleStarClick(starIndex, newScore, label)}
                            onMouseLeave={() => handleMouseLeave(label)}
                        />
                    </div>
                ))}
            {showFeedback && <button onClick={calculateFinalScore}>Afficher le résultat</button>}
        </div>
    );
}
